...
con = pymysql.connect(host="localhost", user="root", password="", database="dbmsproject")
check = 0
...
def enter():
    ...
    def delcar():
        ...
        messagebox.showinfo('success', 'car deleted!!')
            ...
            # Update the car availability, similar to how you handle booking

            # Rest of the code...

        # Add widgets and functionality for leasing a car here




        # Add widgets and functionality for renting a car here

    # creating the main window
    root = Toplevel()
    root.geometry("500x500")
    root.title("CSK RENTALS")
    ...
    def bookcar():
        ...
        f= ent9.get()
        cur = con.cursor()
        cur.execute("insert into users(name,email,startdate,loc,driving_license) values(%s,%s,%s,%s,%s)", (b, c, d, e,f))
        con.commit()
        cur = con.cursor()
    ...
    lab9 = Label(frame2, text="DRIVING LICENSE").place(x=150, y=420)
    driving_license = Entry(frame2)
    driving_license.place(x=250, y=420)

    but3 = Button(frame2, text="BOOK CAR", command=bookcar).place(x=200, y=450)
    b10 = Button(frame2, text="REFRESH", command=enter).place(x=290, y=450)
